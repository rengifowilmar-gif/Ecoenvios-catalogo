...
ARCHIVO_CATALOGO = 'frontend/data/products.json'
ARCHIVO_CANTIDADES = 'frontend/data/quantities.json'
...
def cargar_cantidades(filepath):
    if os.path.exists(filepath):
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                return json.load(f)
...
def guardar_cantidades(datos, filepath):
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(datos, f, ensure_ascii=False, indent=4)
        print(f"[*] Cantidades actualizadas y guardadas en {filepath}")
    except IOError as e:
        print(f"[!] Error guardando cantidades: {e}")
...
async def ejecutar_scraper():
    ...
    cantidades_caja = cargar_cantidades(ARCHIVO_CANTIDADES)
    ...
    async with async_playwright() as p:
                ...
                while True:
                    ...
                    for tarjeta in tarjetas:
                        try:
                            ...
                            # Si el nombre es inválido, tiene puntos suspensivos o capturó basura, lo inferimos de la URL de la imagen
                            if not nombre or len(nombre) < 3 or "..." in nombre or "tiendas d1" in nombre.lower() or len(nombre) > 80 or "Categorias" in nombre or "Inicio" in nombre:
                            ...
                            if precio_actual > 0: 
                                ...
                                if id_producto in catalogo_actual:
                                    # El producto ya existe: Actualizamos datos volátiles e informativos
                                    catalogo_actual[id_producto]["name"] = nombre
                                    ...
                                    # Eliminamos unidadesPorCaja si aún existiera por historial
                                    catalogo_actual[id_producto].pop("unidadesPorCaja", None)
                                else:
                                ...
                                if str_id not in cantidades_caja:
                                    cantidades_caja[str_id] = 1
        ...
        guardar_cantidades(cantidades_caja, ARCHIVO_CANTIDADES)
